...
void rotate(float *input_image, float *output_image, int image_width, int image_height,
    ...
    CHECK_ERROR(err);
    mid = clCreateBuffer(context, CL_MEM_READ_WRITE, image_size, NULL, &err);
    CHECK_ERROR(err);
    ...
    CHECK_ERROR(err);
    err = clEnqueueWriteBuffer(queue, mid, CL_FALSE, 0, image_size, input_image, 0, NULL, NULL);
    CHECK_ERROR(err);

    err = clSetKernelArg(kernel, 0, sizeof(cl_mem), &mid);
    CHECK_ERROR(err);
    ...
    CHECK_ERROR(err);
    err = clSetKernelArg(kernel, 1, sizeof(cl_mem), &mid);
    CHECK_ERROR(err);
    err = clSetKernelArg(kernel, 2, sizeof(cl_int), &image_width);
    CHECK_ERROR(err);
    err = clSetKernelArg(kernel, 3, sizeof(cl_int), &image_height);
